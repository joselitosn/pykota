...
class SQLStorage :
    ...
    def getPrintersUsersAndPQuotas(self, pnames = ["*"], unames=["*"]) :    
        ...
        if result :
            for record in result :
                ...
                if not matchpnames.has_key(pname) :
                    matchpnames[pname] = printermatches = self.tool.matchString(pname, pnames)
                else :    
                    printermatches = matchpnames[pname]
                if printermatches :    
                    if not matchunames.has_key(uname) :
                        matchunames[uname] = usermatches = self.tool.matchString(uname, unames)
                    else :    
                        usermatches = matchunames[uname]
                    if usermatches :    
                        printer = StoragePrinter(self, pname)
    ...
    def getPrintersGroupsAndPQuotas(self, pnames = ["*"], gnames=["*"]) :    
        ...
        groups = {}
        gpquotas = {}
        result = self.doSearch("SELECT printername, groupname FROM printers, groups")
        if result :
            for record in result :
                ...
                if not matchpnames.has_key(pname) :
                    ...
                    printermatches = matchpnames[pname]
                if printermatches :    
                    if not matchgnames.has_key(gname) :
                        matchgnames[gname] = groupmatches = self.tool.matchString(gname, gnames)
                    else :    
                        groupmatches = matchgnames[gname]
                    if groupmatches :    
